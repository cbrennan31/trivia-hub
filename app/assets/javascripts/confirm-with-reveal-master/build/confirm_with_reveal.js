...
(function () {
    ...
    $.fn.extend({
        confirmWithReveal: function (options) {
            ...
            // $el is our element which triggered the call
            do_confirm = function ($el) {
                var confirm_button, confirm_html, confirm_label, el_options, modal, option, password, ref;
                el_options = $el.data('confirm');
                if ($el.attr('data-confirm') == null) {
                    return true;
                }
                // Try and parse single quotes json object:                
                // JSON obj with single quotes == string (not object)
                if ((typeof el_options === 'string') && (el_options.length > 0)) {
                    el_options = el_options.replace(/'/g, '"'); // json doesn't parse single quotes
                    var tmp_el_options = el_options; // back up the el_options
                    try {
                        el_options = JSON.parse(el_options);
                    } catch (e) { // not json
                        el_options = tmp_el_options;
                    }
                }
                // Are we still a string?
                if ((typeof el_options === 'string') && (el_options.length > 0)) {
                    return (((ref = $.rails) != null ? ref.confirm : void 0) || window.confirm).call(window, el_options);
                }
                option = function (name) {
                    return el_options[name] || settings[name];
                };
                modal = $("<div data-reveal class='reveal " + (option('modal_class')) + "' id='confirm-dialog'>\n  <h2 data-confirm-title class='" + (option('title_class')) + "'></h2>\n  <p data-confirm-body class='" + (option('body_class')) + "'></p>\n  <div data-confirm-footer class='" + (option('footer_class')) + "'>\n    <a data-confirm-cancel class='" + (option('cancel_class')) + "'></a>\n  </div>\n</div>");
                ...
                confirm_button.removeAttr('data-confirm').attr('class', option('ok_class')).html(option('ok')).on('click', function (e) {
                    ...
                    if ($el.is('form, :input, button')) {
                        ...
                        // Remove all click event handlers (otherwise submitting form triggers confirm handler again!)
                        // http://stackoverflow.com/questions/825112/how-to-remove-all-click-event-handlers-in-jquery
                        $form.off('submit');
                        $el.off('submit');

                        return $form.removeAttr('data-confirm').submit();
                    }
                    // Success, close the modal:
                    modal.foundation('close');
                    return false; // stop default action, eg. going to '#'
                });
